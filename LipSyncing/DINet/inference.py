...
if __name__ == "__main__":
    ...
    res_video_frame_path_list_pad = (
        [video_frame_path_list_cycle[0]] * 2 + res_video_frame_path_list + [video_frame_path_list_cycle[-1]] * 2
    )
    ...
    res_video_path = os.path.join(
        opt.res_video_dir, os.path.basename(opt.source_video_path)[:-4] + "_facial_dubbing.mp4"
    )
    ...
    for clip_end_index in range(5, pad_length, 1):
        ...
        crop_flag, crop_radius = compute_crop_radius(
            video_size, res_video_landmark_data_pad[clip_end_index - 5 : clip_end_index, :, :], random_scale=1.05
        )
        if not crop_flag:
            raise ("our method can not handle videos with large change of facial size!!")
        crop_radius_1_4 = crop_radius // 4
        ...
        crop_frame_data = frame_data[
            ...
            frame_landmark[33, 0] - crop_radius - crop_radius_1_4 : frame_landmark[33, 0]
            + crop_radius
            + crop_radius_1_4,
            :,
        ]
    ...
    cmd = "ffmpeg -i {} -i {} -c:v libx264 -c:a aac -strict experimental -map 0:v:0 -map 1:a:0 {}".format(
        res_video_path, opt.driving_audio_path, video_add_audio_path
    )
    subprocess.call(cmd, shell=True)

    rmtree(video_frame_dir)
    os.remove(res_video_path)
    os.remove(res_face_path)
    output_fpath = os.path.join(opt.res_video_dir, os.path.basename(opt.source_video_path))
    move(video_add_audio_path, output_fpath)
