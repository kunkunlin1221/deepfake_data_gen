...
def crop_with_pad(image, x1, y1, x2, y2, pad_value=0):
    h, w = image.shape[:2]
    # Calculate padding
    pad_top = max(0, -y1)
    ...
    padded_image = cv2.copyMakeBorder(
        image, pad_top, pad_bottom, pad_left, pad_right, cv2.BORDER_CONSTANT, value=pad_value
    )
    cropped_image = padded_image[pad_top + y1 : pad_top + y2, pad_left + x1 : pad_left + x2]
...
def main(args):
    ...
    vae, unet, pe = load_all_model(
        unet_model_path=args.unet_model_path, vae_type=args.vae_type, unet_config=args.unet_config, device=device
    )
    ...
    for task_id in inference_config:
        try:
            ...
            for bbox, frame in zip(coord_list, frame_list):
                ...
                crop_frame = crop_with_pad(frame, x1, y1, x2, y2, pad_value=0)
            ...
            shutil.move(output_vid_name, "tmp.mp4")
            shutil.rmtree(args.result_dir)
            shutil.move("tmp.mp4", args.result_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ffmpeg_path", type=str, default="./ffmpeg-4.4-amd64-static/", help="Path to ffmpeg executable"
    )
    parser.add_argument("--gpu_id", type=int, default=0, help="GPU ID to use")
    parser.add_argument("--vae_type", type=str, default="sd-vae", help="Type of VAE model")
    parser.add_argument(
        "--unet_config", type=str, default="./models/musetalk/config.json", help="Path to UNet configuration file"
    )
    parser.add_argument(
        "--unet_model_path", type=str, default="./models/musetalkV15/unet.pth", help="Path to UNet model weights"
    )
    parser.add_argument(
        "--whisper_dir", type=str, default="./models/whisper", help="Directory containing Whisper model"
    )
    parser.add_argument(
        "--inference_config",
        type=str,
