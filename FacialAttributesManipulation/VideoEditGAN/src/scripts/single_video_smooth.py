...
def main(args):
    ...
    video_path = args.video_fp
    reader = sorted(glob.glob(os.path.join(video_path, "*.png")))
    # reader = sorted(glob.glob(os.path.join(video_path, '*.jpg')) + glob.glob(os.path.join(video_path, '*.jpeg')))
    queue_ver = deque()
    queue_frame = deque()
    final_ver = []
    # run
    ...
    for i, frame_path in enumerate(reader):
        ...
        if len(queue_ver) >= n:
            ...
            final_ver.append(ver_ave[0:2].transpose(1, 0))
    ...
    for _ in range(n_next):
        ...
        queue_ver.popleft()
        queue_frame.popleft()
    final_ver = np.stack(final_ver)
    output_root = os.path.join(args.video_fp, "../landmarks")
    os.makedirs(output_root, exist_ok=True)
    output_path = os.path.join(output_root, "landmarks.npy")
    np.save(output_path, final_ver)
