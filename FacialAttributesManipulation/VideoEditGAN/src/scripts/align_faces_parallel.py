...
def align_face(filepath, ld=None, output_size=1024, transform_size=1024, output_quad=False):
    """
    :param filepath: str
    :return: PIL Image
    """
    ...
    if not output_quad:
        ...
        return img, quad + 0.5, crop_pad_size, crop
...
def extract_on_paths(file_paths):
    ...
    count = 0
    lds = np.load(os.path.join(os.path.dirname(file_paths[0][0]).replace("frames", "landmarks"), "landmarks.npy"))
    for file_path, res_path in sorted(file_paths):
        count += 1
        if count % 100 == 0:
            print("{} done with {}/{}".format(pid, count, tot_count))

        try:
            res = align_face(file_path, ld=lds[count - 1])
            res = res.convert("RGB")
...
def run(root_path, output_path=None, num_threads=1):
    if output_path is None:
        out_crops_path = root_path + "_crops"
    else:
        out_crops_path = output_path
...
if __name__ == "__main__":
    ...
    run(args.root_path, args.output_path, args.num_threads)
