...
class LitModel(pl.LightningModule):
    ...
    def encode_stochastic(self, x, cond, T=None, use_ema=True, cond_other=None):
        ...
        if use_ema:
            out = sampler.ddim_reverse_sample_loop(
                self.ema_model, x, model_kwargs={"cond": cond, "cond_other": cond_other}
            )
        else:
            out = sampler.ddim_reverse_sample_loop(self.model, x, model_kwargs={"cond": cond, "cond_other": cond_other})
        return out["sample"]
    ...
    def infer_whole_dataset(self, with_render=False, T_render=None, render_save_path=None):
        ...
        """
        loader = self.conf.make_loader(self.val_data, shuffle=True, drop_last=False)
    ...
    def log_sample(self, x_start):
        ...
        def do(model, postfix, use_xstart, save_real=False):
            model.eval()
            ...
            if self.x0_estimated is not None:
                ...
                if self.global_rank == 0:
                    ...
                    sample_dir = os.path.join(self.conf.logdir, f"x0_estimated{postfix}")
            ...
            with torch.no_grad():
                ...
                cond_other = None
                stochastic_x_T = self.encode_stochastic(
                    x_start, cond, use_ema=("ema" in postfix), cond_other=cond_other
                )
                ...
                gen = self.eval_sampler.sample(
                    model=model, noise=x_T, cond=cond, x_start=_xstart, cond_other=cond_other
                )

                x_T = random_x_T
                gen_random = self.eval_sampler.sample(
                    model=model, noise=x_T, cond=cond, x_start=_xstart, cond_other=cond_other
                )

                x_T = stochastic_x_T
                cond_decom = model.encoder.forward(id=_xstart, lnd=self.last_real)
                gen_decom = self.eval_sampler.sample(
                    model=model, noise=x_T, cond=cond_decom, x_start=_xstart, cond_other=None
                )
                ...
                if save_real and use_xstart:
                    ...
                    if self.global_rank == 0:
                        ...
                        real_sample_dir = os.path.join(self.conf.logdir, f"real_sample{postfix}")
                        if not os.path.exists(real_sample_dir):
                            os.makedirs(real_sample_dir)
                        path = os.path.join(real_sample_dir, "%d.png" % self.num_samples)
                        save_image(grid_real, path)
                        if gen_decom is not None:
                            grid_real = (make_grid(last_real) + 1) / 2
                            self.logger.experiment.add_image(f"sample{postfix}/last_batch", grid_real, self.num_samples)
                            path = os.path.join(real_sample_dir, "%d_last_batch.png" % self.num_samples)
                            save_image(grid_real, path)
                ...
                if self.global_rank == 0:
                    ...
                    if not os.path.exists(sample_dir):
                        os.makedirs(sample_dir)
                    path = os.path.join(sample_dir, "%d_recon.png" % self.num_samples)
                    save_image(grid, path)
                    self.logger.experiment.add_image(f"sample{postfix}/recon", grid, self.num_samples)
                    grid = (make_grid(gen_random) + 1) / 2
                    path = os.path.join(sample_dir, "%d_rand_xT.png" % self.num_samples)
                    save_image(grid, path)
                    self.logger.experiment.add_image(f"sample{postfix}/rand_xT", grid, self.num_samples)
                    if gen_decom is not None:
                        grid = (make_grid(gen_decom) + 1) / 2
                        path = os.path.join(sample_dir, "%d_decom.png" % self.num_samples)
                        save_image(grid, path)
                        self.logger.experiment.add_image(f"sample{postfix}/decom", grid, self.num_samples)
            model.train()
