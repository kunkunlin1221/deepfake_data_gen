...
def make_subset_loader(
    conf: TrainConfig, dataset: Dataset, batch_size: int, shuffle: bool, parallel: bool, drop_last=True
):
...
def evaluate_lpips(
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(val_loader, desc="lpips"):
            ...
            if conf.model_type == ModelType.ddpm:
                ...
                pred_imgs = render_condition(conf=conf, model=model, x_T=x_T, x_start=imgs, cond=None, sampler=sampler)
            # # returns {'cond', 'cond2'}
            ...
            # (n, )
            scores["mse"].append((norm_imgs - norm_pred_imgs).pow(2).mean(dim=[1, 2, 3]))
            # (n, )
    ...
    # support multi-gpu
    outs = {
        key: [torch.zeros(len(scores[key]), device=device) for i in range(get_world_size())] for key in scores.keys()
    }
    for key in scores.keys():
        ...
        if conf.model_type.can_sample():
            ...
            for i in trange(0, eval_num_images, batch_size, desc=desc):
                ...
                for j in range(len(batch_images)):
                    ...
                    torchvision.utils.save_image(batch_images[j], os.path.join(conf.generate_dir, f"{img_name}.png"))
            ...
            if conf.train_mode.is_latent_diffusion():
                ...
                for i in trange(0, eval_num_images, batch_size, desc=desc):
                    ...
                    for j in range(len(batch_images)):
                        ...
                        torchvision.utils.save_image(
                            batch_images[j], os.path.join(conf.generate_dir, f"{img_name}.png")
                        )
            else:
                ...
                for batch in tqdm(train_loader, desc="generating images"):
                    ...
                    batch_images = render_condition(
                        conf=conf, model=model, x_T=x_T, x_start=imgs, cond=None, sampler=sampler
                    ).cpu()  # , EDIT
