...
class GaussianDiffusionBeatGans:
    ...
    def training_losses(
        self, model: Model, x_start: th.Tensor, t: th.Tensor, model_kwargs=None, noise: th.Tensor = None
    ):
        """
        ...
        if self.loss_type in [
            ...
            with autocast(self.conf.fp16):
                ...
                model_forward = model.forward(
                    x=x_t.detach(), t=self._scale_timesteps(t), x_start=x_start.detach(), **model_kwargs
                )
            ...
            with autocast(self.conf.fp16):
                # x_t is static wrt. to the diffusion process
                model_forward2 = model.forward(
                    x=x_t2.detach(), t=self._scale_timesteps(t), x_start=x_start.detach(), **model_kwargs
                )
            ...
            if self.loss_type == LossType.mse:
                if self.model_mean_type == ModelMeanType.eps:
                    ...
                    terms["xT_reg"] = (
                        mean_flat((terms["x0_estimated"] - terms["x0_estimated2"]) ** 2) * self.conf.xT_reg_coef
                    )
                else:
                    raise NotImplementedError()
            elif self.loss_type == LossType.l1:
                ...
                terms["xT_reg"] = (
                    mean_flat((terms["x0_estimated"] - terms["x0_estimated2"]).abs()) * self.conf.xT_reg_coef
                )
            else:
                raise NotImplementedError()
    ...
    def ddim_sample(
        ...
        alpha_bar = _extract_into_tensor(self.alphas_cumprod, t, x.shape)
        alpha_bar_prev = _extract_into_tensor(self.alphas_cumprod_prev, t, x.shape)  # at_next
